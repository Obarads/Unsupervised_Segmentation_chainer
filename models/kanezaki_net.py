...
class KanezakiNet(chainer.Chain):
    def __init__(self, input_dim, nChannel=100, nConv=2):
        ...
        with self.init_scope():
            ...
            self.conv_b1 = ConvBlock1(input_dim, nChannel, ksize=3, stride=1, pad=1, processing_order=["act","bn"])
            self.conv_b2 = chainer.ChainList()
            for i in range(nConv-1):
                #self.conv_b2.append(ConvBlock(nChannel, nChannel, ksize=3, stride=1, pad=1, use_bn=True))
                self.conv_b2.append(ConvBlock1(nChannel,nChannel,ksize=3,stride=1,pad=1,processing_order=["act","bn"]))
            #self.conv_b3 = ConvBlock(nChannel, nChannel, ksize=1, stride=1, pad=0, use_bn=True, activation=None)
            self.conv_b3 = ConvBlock1(nChannel,nChannel, ksize=1, stride=1, pad=0, processing_order=["bn"])
